!(function (e, n) {
  ...
  return class {
    constructor() {
      (this._internal = { emitter: new e(), device: null, profile: null }),
        navigator.usb.addEventListener("disconnect", (e) => {
          this._internal.device == e.device &&
    ...
    async connect() {
      try {
        let e = await navigator.usb.requestDevice({
          filters: n.map((e) => e.filters).reduce((e, n) => e.concat(n)),
        });
        e && (await this.open(e));
      } catch (e) {
    ...
    async reconnect(e) {
      let n = await navigator.usb.getDevices(),
        i = n.find((n) => n.serialNumber == e.serialNumber);
      i ||
        (i = n.find(
    ...
    async open(e) {
      (this._internal.device = e),
        (this._internal.profile = n.find((e) =>
          e.filters.some((e) =>
            e.vendorId && e.productId
              ? e.vendorId == this._internal.device.vendorId &&
                e.productId == this._internal.device.productId
              : e.vendorId == this._internal.device.vendorId
          )
        )),
        await this._internal.device.open(),
        await this._internal.device.selectConfiguration(
          this._internal.profile.configuration
        ),
        await this._internal.device.claimInterface(
          this._internal.profile.interface
        ),
    ...
    async print(e) {
      let n = this._internal.profile.endpoint;
      if (!n) {
        let e = this._internal.device.configuration.interfaces
          .find((e) => e.interfaceNumber == this._internal.profile.interface)
          .alternate.endpoints.find((e) => "out" == e.direction);
        e && (n = e.endpointNumber);
      }
      if (n)
        try {
          await this._internal.device.transferOut(n, e);
        } catch (e) {
          console.log(e);
        }
    }
    addEventListener(e, n) {
      this._internal.emitter.on(e, n);
    }
  };
});
